    ...
    constructor() {
        ...
        vec3 getColorByPercent(float pct){\
            vec3 v3=vec3(0.0,0.0,pct);\
                  ...
                  if(pct <= 0.00){\
        return vec3(0.263, 0.573, 0.996); /* #4392fe */\
    }else if(pct <= 0.08){\
        float t = (pct - 0.00) / (0.08 - 0.00); \
        return mix(vec3(0.263, 0.573, 0.996), vec3(0.306, 0.675, 1.000), t); /* #4eacff */\
    }else if(pct <= 0.25){\
        float t = (pct - 0.08) / (0.25 - 0.08); \
        return mix(vec3(0.306, 0.675, 1.000), vec3(0.365, 0.796, 1.000), t); /* #5dcbff */\
    }else if(pct <= 0.33){\
        float t = (pct - 0.25) / (0.33 - 0.25); \
        return mix(vec3(0.365, 0.796, 1.000), vec3(0.392, 0.906, 0.898), t); /* #64e7e5 */\
    }else if(pct <= 0.42){\
        float t = (pct - 0.33) / (0.42 - 0.33); \
        return mix(vec3(0.392, 0.906, 0.898), vec3(0.471, 0.988, 0.702), t); /* #78fcb3 */\
    }else if(pct <= 0.50){\
        float t = (pct - 0.42) / (0.50 - 0.42); \
        return mix(vec3(0.471, 0.988, 0.702), vec3(0.694, 0.980, 0.553), t); /* #b1fa8d */\
    }else if(pct <= 0.58){\
        float t = (pct - 0.50) / (0.58 - 0.50); \
        return mix(vec3(0.694, 0.980, 0.553), vec3(1.000, 0.855, 0.396), t); /* #ffda65 */\
    }else if(pct <= 0.67){\
        float t = (pct - 0.58) / (0.67 - 0.58); \
        return mix(vec3(1.000, 0.855, 0.396), vec3(1.000, 0.784, 0.345), t); /* #ffc858 */\
    }else if(pct <= 0.75){\
        float t = (pct - 0.67) / (0.75 - 0.67); \
        return mix(vec3(1.000, 0.784, 0.345), vec3(1.000, 0.702, 0.325), t); /* #ffb353 */\
    }else if(pct <= 0.81){\
        float t = (pct - 0.75) / (0.81 - 0.75); \
        return mix(vec3(1.000, 0.702, 0.325), vec3(1.000, 0.588, 0.376), t); /* #ff9660 */\
    }else if(pct <= 0.92){\
        float t = (pct - 0.81) / (0.92 - 0.81); \
        return mix(vec3(1.000, 0.588, 0.376), vec3(1.000, 0.506, 0.408), t); /* #ff8168 */\
    }else{\
        return vec3(1.000, 0.506, 0.408); /* fallback #ff8168 */\
    }\
            return v3;\
        }\
        void main(void){\
...
// document.appendChild(tplCanvas)
